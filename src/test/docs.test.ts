...
describe('docs Package', () => {
    ...
    `)
		...
		expect(modifiers).toStrictEqual(['email()', 'optional()', 'url()', 'uuid()', 'min(12)', 'refine((val) => val !== 14)'])
	})
	...
	test('getJSDocs', () => {
		const docLines = getJSDocs(['This is something', 'How about something else', '@something', '@example ur mom'].join('\n'))
